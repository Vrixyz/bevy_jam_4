...
pub(crate) fn spawn_layout(mut commands: Commands, mut rng: ResMut<crate::RandomDeterministic>) {
    let inventory = vec![
...
fn get_random_item(rng: &mut ResMut<'_, crate::RandomDeterministic>) -> ItemType {
    let choices_mesh = [
        (MeshType::Triangle, 2),
        (MeshType::Circle, 2),
        (MeshType::Quad, 2),
